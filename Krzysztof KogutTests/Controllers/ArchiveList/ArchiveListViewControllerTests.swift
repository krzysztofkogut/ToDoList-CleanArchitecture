//
//  ArchiveListViewControllerTests.swift
...
class ArchiveListViewControllerTests: XCTestCase {
    let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
    func testInitWithCoder() {
        XCTAssertNil(ArchiveListViewController(coder: NSKeyedArchiver(requiringSecureCoding: true)))
    }
    ...
    func testNumberOfRowsInSection() {
        ...
        let view = ArchiveListView()
        let viewModel = ArchiveListViewModelMock(archiveLists: [list1, list2])
        let router = ArchiveListRouterMock()
        let controller = ArchiveListViewController(
            view: view,
            viewModel: viewModel,
            router: router
        )
